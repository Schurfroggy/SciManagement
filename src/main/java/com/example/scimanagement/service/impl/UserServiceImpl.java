...
public class UserServiceImpl extends ServiceImpl<userMapper, User> implements IUserService {
    ...
    public Result login(LoginFormDTO loginForm) {
        ...
        if(!Boolean.TRUE.equals(redisTemplate.hasKey(phone))){
            ...
            redisTemplate.opsForHash().put(phone,"id",user.getId().toString());
        ...
        if(!redisTemplate.opsForHash().get(phone,"password").equals(loginForm.getPassword())){
            return fail("密码错误");
        }
        User user = new User();
        user.setId((Integer) redisTemplate.opsForHash().get(phone,"id"));
